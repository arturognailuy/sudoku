...
15. `.aidoc/designs/database-puzzle-selection.md` — proposed acquisition, recycling, and migration contract
16. `db/db.go` — SQLite database open/close/migrate
17. `db/puzzle.go` — puzzle CRUD, random query by difficulty, dedup
18. `cmd/play.go` — fallback flow (generator → DB lookup → graceful degradation) and auto-store
19. `cmd/generate.go` — batch generation CLI (parallel workers, progress, report)
20. `cmd/import.go` — import CLI (file parsing, normalization, dedup, report)
...
5. `.aidoc/designs/database-puzzle-selection.md` — proposed next database behavior, migration, and acceptance boundary
6. `.aidoc/designs/future-directions.md` — deliberately non-priority product and production directions
7. `.aidoc/designs/web-api.md` — client-neutral HTTP resources, revisions, recovery, client access, and security boundary
8. `api/openapi.yaml` — canonical OpenAPI 3.1.1 wire contract, schemas, errors, and examples
9. `.aidoc/designs/game-engine.md` — stable engine API, notes, history, and serialization design
10. `.aidoc/designs/background-autosave.md` — recovery lifecycle, privacy, storage, retention, and conflict policy
11. `.aidoc/designs/tui-frontend.md` — current full-screen interaction and rendering semantics
12. `.aidoc/architecture/guidelines.md` — current architecture and solver contract
...
| `.aidoc/designs/database-puzzle-selection.md` | Proposed exact-grade acquisition, played-state recycling, migration, and acceptance contract |
