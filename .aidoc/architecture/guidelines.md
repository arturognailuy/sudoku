...
Defined in `solver/solver.go`. All solvers implement `Solver` with these semantics:
...
- **`CountSolutions(board)`** — only meaningful for reliable solvers. Unreliable solvers inherit `Base.CountSolutions` which returns `0`.

`Base` (`solver/solver.go`) provides default implementations for metadata methods and `CountSolutions`. Strategy solvers embed `Base` with `Reliable: false`.

## Adding a New Solver

1. Create `solver/<technique>_solver.go` embedding `Base`.
2. Implement `Solve()` and `Hint()`. Inherit `CountSolutions()` from `Base`.
3. Register in `solver.NewStore()`.
...
- **Interface naming:** Types follow Go conventions — no `I` prefix, no redundant package-name prefixes. `Solver` (interface), `Base`, `Backtracker`, `Store`, `Board`, `Game`, `Difficulty`, etc.
